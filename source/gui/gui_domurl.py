...
import src.scripts.urleodactyl as urldct
import src.utils.webview as webvw
import src.utils.xl_generator as xl_gen

class GuiUrl(QtWidgets.QMainWindow):
    gui_win = None
    statusinfo = None
    cache_data = None
    ...
    def __init__(self):
        ...
        self.gui_win.lb_iplist.setText("Domain/URL list:")
        self.gui_win.fr_inp.setStyleSheet("QFrame{ background-image: url(images/betexedit.png); }")
    ...
    def submit_clicked(self):
        ...
        self.startThreadUrl(given_ioclist)

    def toxl_clicked(self) :
        print("INFO: toxl clicked")
        exported = xl_gen.exportToXl(self.cache_data, 2)
        if exported :
    ...
    def startThreadUrl(self, given_urllist) :
        if (len(given_urllist)) > 2 :
            self.thread = QThread()
            self.worker = urldct.Urleodactyl()
            self.worker.moveToThread(self.thread)
            self.thread.started.connect(self.worker.run)
            self.worker.finished.connect(self.thread.quit)
            self.worker.finished.connect(self.worker.deleteLater)
            self.thread.finished.connect(self.thread.deleteLater)
            self.worker.setLines(given_urllist)
            self.thread.start()
    ...
    def setTable(self, row, cols) :
        ...
        table.setHorizontalHeaderLabels([
            'URL','Reputation','Community','Times submitted',
            'PassiveDNS', 'Kaspersky','BitDefender','Sophos'
        ])
    ...
    def onOtxClicked(self) :
        ...
        if col_cioc == 0 and row_cioc >= 0:
            try :
                ...
                self.ui.setupUi(2, self.dialog, got_cioc[0])
