...
class GuiIpAddr(QtWidgets.QMainWindow):
    gui_win = None
    statusinfo = None
    cache_data = None
    ...
    def __init__(self):
        ...
        self.gui_win.frame4borders.setStyleSheet("QWidget{ background-image: url(images/framebord.png); background:transparent; }")
    ...
    def setTable(self, row) :
        ...
        table.setHorizontalHeaderLabels([
            'IP address','Country','Domain','ISP','UsageType',
            'Tirexfactor','AbuseIPDB','VirusTotal','IBM XForce',
            'MetaDefender','GreyNoise','Pulsedive', 'VTCommunity',
            'Shodan openPorts', 'Shodan CSBeacon', 'Hosted URLs'
        ])
    ...
    def setTheGraph(self, countryList) :
        graficus = self.gui_win.graphc
        self.gui_win.frame4borders.show()
        graficus.show()
    ...
    def onOtxClicked(self) :
        ...
        if col_cioc == 0 and row_cioc >= 0:
            try :
                ...
                self.ui.setupUi(1, self.dialog, got_cioc[0])
